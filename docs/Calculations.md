...
- **Rows already in the display currency** show their original amount and need no rate, one at a time (`FormatWithOriginal`) and in `TrySumDisplayFromUSD` totals. `FormatTotalOrPending` totals and reports convert every row from USD.

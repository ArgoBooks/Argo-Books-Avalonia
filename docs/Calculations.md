...
**Imported invoices.** The spreadsheet importer takes the sheet's status, reading Overdue as Sent. A sheet that says Draft, Cancelled, Refunded or PartiallyRefunded is taken at its word whatever the amounts, since none of those can be worked out from an amount paid: a cancelled invoice with a payment stays Cancelled, and so is never overdue. Any other status is then put through `RecalculateStatus` with the imported amounts, as if the amount paid had arrived as a payment. So an Overdue or Sent invoice with 50 of 100 paid becomes Partial, one paid in full becomes Paid, and an unpaid Pending, Sent or Viewed invoice keeps its status. A row that gives no status is read as Draft, but only until its amounts are worked out, so one paid in full still becomes Paid. A status counts as given only when it is one of the statuses above, in any case and ignoring spaces, hyphens and underscores, or one of a few other spellings: Canceled is Cancelled, Paid in full is Paid, and Partially paid is Partial. Anything else, such as Open or Unpaid, is read as giving no status, so the amounts decide it (`SpreadsheetImportService.ParseImportedInvoiceStatus`). A sheet that updates existing invoices with a Status column and no amount columns, such as one marking a batch paid, sets the status as given, except that Overdue is still worked out from the stored amounts.

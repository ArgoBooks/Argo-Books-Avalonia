...
**Updating an existing record.** Both imports change only what the sheet gives. The column import changes the fields it has columns for, and the AI import the fields its row gives a value for (`RecordLists.FillAbsent`), whatever the record: an invoice, a revenue, an expense, a payment, a customer, and so on. So a sheet that only updates the status of existing invoices leaves their amounts as they are, one that gives only a new total keeps what was already paid, and a record's USD amounts are converted again only when something they are worked out from (a date, an amount or the currency) is given. A part of a record made of several fields, such as an address, is updated field by field too: a sheet with only a City column, or an AI row giving only the city, keeps the street and country.

A revenue's or expense's lines follow one rule in both imports (`SpreadsheetImportService.SetSingleLine`). A new record gets one line for the product its description names. An existing record with one line, or none, has that line rebuilt when the row gives any field it is made from (the product or description, quantity, unit price, tax or pre-tax amount), keeping the line's own values for the ones the row leaves out; a row that gives none of them leaves the lines alone. A product or description that is still the line's own description, or the current name of its product (ignoring case and surrounding spaces), keeps the line's product. The product is looked up by name only when the text changed, so re-importing the app's own export, whose Product column is the record's description, doesn't move a sale onto another product when its product was renamed since or shares its name with another. A record with several lines keeps them, since one row can't describe them. Its pre-tax amount, when the row gives a quantity or unit price, is its lines' subtotal (`InvoiceMath.Subtotal`), as when the app saves it; for a record with one line or none it is quantity × unit price. The export writes a multi-line record's total quantity and average unit price, and their product is not the lines' sum. A revenue that names an invoice takes the invoice's lines only when it is new, has no lines, or the row moves it to another invoice; an existing sale keeps its own lines and the cost of goods sold they carry. How a changed line affects stock is in §14.
...
A payment can't be recorded on a Draft invoice, because a draft has no revenue until it is sent. A sheet can still give a Draft invoice an amount paid. The import then treats that amount as a payment: the invoice is no longer a draft (§6), and it gets its revenue like any other invoice imported with an amount paid.
...
**Imported invoices.** The spreadsheet importer takes the sheet's status, reading Overdue as Sent. Cancelled, Refunded and PartiallyRefunded can't be worked out from an amount paid, so one the sheet gives is taken at its word whatever the amounts, and so is one an existing invoice already has when the sheet gives no status: a cancelled invoice with a payment stays Cancelled, and so is never overdue. Every other status, Draft included, is one the amounts decide. The invoice starts from the status it had before any payment (`StatusBeforePayment`, or Sent when it has none), and `RecalculateStatus` moves it on with the imported amounts, as if the amount paid had arrived as a payment. So an Overdue or Sent invoice with 50 of 100 paid becomes Partial, one paid in full becomes Paid, an unpaid Draft, Pending, Sent or Viewed invoice keeps its status, a Draft with an amount paid becomes Partial or Paid, as `RecalculateStatus` moves a Draft on at its first payment, and one marked Paid or Partial with nothing paid is Sent and owes its total. A row for a new invoice that gives no status is read as Draft, so one paid in full still becomes Paid. A row that updates an existing invoice and gives no status keeps that invoice's own status, which its amounts then work out the same way when the sheet gives any. A status counts as given only when it is one of the statuses above, in any case and ignoring spaces, hyphens and underscores, or one of a few other spellings: Canceled, Void and Voided are Cancelled, Paid in full is Paid, and Partially paid is Partial. Anything else, such as Open or Unpaid, is read as giving no status, so the amounts decide it (`SpreadsheetImportService.ParseImportedInvoiceStatus`). A sheet that updates existing invoices with a Status column and no amount columns, such as one marking a batch paid, sets the status as given, except that Overdue is still worked out from the stored amounts.
...
**A spreadsheet import updating a sale or purchase.** The import moves no stock for the records it brings in, even when a stock adjustment already names a new record's id (a company restored from an export, or an id used again). When it changes the lines of an existing one (§5), a line whose product and quantity stay the same, and for a purchase its unit price too, keeps the stock it took and its cost of goods sold as saved. Otherwise, a record that moved stock or whose lines say what stock they took is applied to stock again exactly as editing it in the app is (`InventoryStockService.ApplyEdit`), so its cost of goods sold is worked out afresh rather than lost, and a record that never moved stock stays out of it. A purchase that bought tracked stock is applied again the same way when the row changes its date or currency, even if its lines stay the same or it has several, because its stock's unit cost is converted at the purchase's own date and currency, and editing either in the app costs the stock again too.

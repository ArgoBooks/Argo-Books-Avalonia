...
**A spreadsheet import updating a sale or purchase.** The import moves no stock for the records it brings in. When it changes the lines of an existing one (§5), a line whose product and quantity stay the same, and for a purchase its unit price too, keeps the stock it took and its cost of goods sold as saved. Otherwise, a record that moved stock or whose lines say what stock they took is applied to stock again exactly as editing it in the app is (`InventoryStockService.ApplyEdit`), so its cost of goods sold is worked out afresh rather than lost, and a record that never moved stock stays out of it.
